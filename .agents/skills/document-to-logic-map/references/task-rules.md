...
1. `core_judgment_group`, `judgment_basis_group`, and `potential_action_group` are conditional visible groups, not mandatory placeholders.
2. If a judgment group has no concrete descendants, omit the group instead of keeping a note-only fallback shell.
3. If a judgment module ends up with no concrete descendants after pruning unsupported groups, omit the whole module.
4. Unsupported roles must be omitted. Do not fabricate repaired basis/action children just to keep the visible skeleton complete.
